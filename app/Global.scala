...
object Global extends GlobalSettings {
	...
	override def onRouteRequest (request: RequestHeader) = getSubdomain(request) match {
		case "manager" => manager.Routes.routes.lift(request)
		case _ => client.Routes.routes.lift(request)
	}
	
	// 404 - page not found error
	override def onHandlerNotFound (request: RequestHeader) = getSubdomain(request) match {
		case "manager" => ManagerGlobal.onHandlerNotFound(request)
		case _ => ClientGlobal.onHandlerNotFound(request)
	}
	
	// 500 - internal server error
	override def onError (request: RequestHeader, throwable: Throwable) = getSubdomain(request) match {
		case "manager" => ManagerGlobal.onError(request, throwable)
		case _ => ClientGlobal.onError(request, throwable)
	}
	
	// called when a route is found, but it was not possible to bind the request parameters
	override def onBadRequest (request: RequestHeader, error: String) = getSubdomain(request) match {
		case "manager" => ManagerGlobal.onBadRequest(request, error)
		case _ => ClientGlobal.onBadRequest(request, error)
	}
