 ...
 * @param List
 *   of SQL strings comprising a clause. It can be any valid statement, which should contain bind placeholders [?]. For example, ["my_field1 \= ?", "my_field2 =
 *   ?"]
 *
 * Clause components are not full and complete SQL statements. They are used as an intermediate artifact while a query is being built.
 *
