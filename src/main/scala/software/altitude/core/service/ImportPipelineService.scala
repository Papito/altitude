...
class ImportPipelineService(app: Altitude) {
  ...
  private def runAsQueue(): Sink[TDataAssetWithContext, NotUsed] = {
    logger.info("Starting the import queue pipeline")
    val bufferSize = parallelism * 2
    val (mergeHubSink, mergeHubSource) = MergeHub.source[TDataAssetWithContext](perProducerBufferSize = bufferSize).preMaterialize()
    val streamFuture = mergeHubSource
      .via(combinedFlow)
      .toMat(Sink.foreach(n => println(s"Completed processing: $n")))(Keep.right)
      .run()

    streamFuture.onComplete {
      case Success(_) =>
        println(s"[${java.time.LocalDateTime.now}] Stream completed successfully")
      case Failure(ex) =>
        println(s"[${java.time.LocalDateTime.now}] Stream failed: ${ex.getMessage}")
    } (system.executionContext)

    mergeHubSink
  }

  def addToQueue(asset: TDataAssetWithContext) : NotUsed = {
    Source.single(asset).runWith(queueSink)
  }
