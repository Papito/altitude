...
@DoNotDiscover class AssetQueryTests(val config: Map[String, Any]) extends IntegrationTestCore {
  test("Empty search") {
    val results = altitudeApp.service.library.query(new Query())
  ...
  test("Search all") {
    ...
    val assets = altitudeApp.service.library.query(new Query()).records
  ...
  test("Pagination") {
    ...
    val results = altitudeApp.service.library.query(q)
