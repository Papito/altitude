...
@DoNotDiscover class FaceDetectionTests(override val testApp: Altitude) extends IntegrationTestCore {
  ...
  test("Face is detected in an image (1)") {
    ...
    val detections = testApp.service.faceDetection.detectFacesWithYunet(FaceDetectionService.matFromBytes(importAsset.data))
  ...
  test("Faces are detected in an image (1)") {
    ...
    val detections = testApp.service.faceDetection.detectFacesWithDnnNet(FaceDetectionService.matFromBytes(importAsset.data))
  ...
  test("Large portrait face image is detected (1)") {
    ...
    val imageMat = FaceDetectionService.matFromBytes(importAsset.data)
  ...
  test("Faces from the same image are identical") {
    val importAsset = IntegrationTestUtil.getImportAsset("people/meme-ben.jpg")
    val imageMat1: Mat = FaceDetectionService.matFromBytes(importAsset.data)
  ...
  test("Same person is identified as same in multiple images (1)") {
    ...
    val imageMat2: Mat = FaceDetectionService.matFromBytes(importAsset2.data)
    ...
    val imageMat3: Mat = FaceDetectionService.matFromBytes(importAsset3.data)
