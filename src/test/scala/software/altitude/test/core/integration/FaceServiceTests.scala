...
@DoNotDiscover class FaceServiceTests(override val testApp: Altitude) extends IntegrationTestCore {
  ...
  test("Faces are detected in an image (1)", Focused) {
    ...
    val detections = FaceService.detectFacesWithDnnNet(FaceService.matFromBytes(importAsset.data))
  ...
  test("Faces from the same image are identical", Focused) {
    ...
    val detections = FaceService.detectFacesWithYunet(imageMat1)
  ...
  test("Same person is identified as same in multiple images (1)", Focused) {
    ...
    val detections2 = FaceService.detectFacesWithYunet(imageMat2)
    ...
    val detections3 = FaceService.detectFacesWithYunet(imageMat3)
    val detection3 = detections3.head

    // 1 & 2
    FaceService.isFaceSimilar(imageMat1, imageMat2, detection1, detection2) should be(true)
    // 1 & 3
    FaceService.isFaceSimilar(imageMat1, imageMat3, detection1, detection3) should be(true)
    // 2 & 3
    FaceService.isFaceSimilar(imageMat2, imageMat3, detection2, detection3) should be(true)
  }
  ...
  test("Different people are NOT identified as same in the same image (1)", Focused) {
    ...
    FaceService.isFaceSimilar(imageMat1, imageMat1, detection1, detection2) should be(false)
  }
  ...
  test("Different people are NOT identified as same in multiple images (1)", Focused) {
    ...
    // 1 & 2
    FaceService.isFaceSimilar(imageMat1, imageMat2, detection1, detection2) should be(false)
    // 1 & 3
    FaceService.isFaceSimilar(imageMat1, imageMat3, detection1, detection3) should be(false)
    // 2 & 3
    FaceService.isFaceSimilar(imageMat2, imageMat3, detection2, detection3) should be(false)
  }
}
