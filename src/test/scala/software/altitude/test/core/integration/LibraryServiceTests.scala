...
@DoNotDiscover class LibraryServiceTests(val config: Map[String, Any]) extends IntegrationTestCore {
  ...
  test("Folder counts should check out") {
    ...
    */
    ...
    val folder2_2_1: Folder = altitudeApp.service.library.addFolder(
      name = "folder2_2_1", parentId = folder2_2.id)

    val folder2_2_2: Folder = altitudeApp.service.library.addFolder(
    ...
    val all = altitudeApp.service.folder.repositoryFolders()

    // test counts for individual folders
    (altitudeApp.service.folder.getByIdWithChildAssetCounts(folder1.persistedId, all): Folder).numOfAssets shouldBe 2
    (altitudeApp.service.folder.getByIdWithChildAssetCounts(folder2_2_1.persistedId, all): Folder).numOfAssets shouldBe 2
    (altitudeApp.service.folder.getByIdWithChildAssetCounts(folder2_2_2.persistedId, all): Folder).numOfAssets shouldBe 2
    (altitudeApp.service.folder.getByIdWithChildAssetCounts(folder2_2.persistedId, all): Folder).numOfAssets shouldBe 6
    (altitudeApp.service.folder.getByIdWithChildAssetCounts(folder2_1.persistedId, all): Folder).numOfAssets shouldBe 2
    (altitudeApp.service.folder.getByIdWithChildAssetCounts(folder2.persistedId, all): Folder).numOfAssets shouldBe 10

    // test counts for immediate children
    val rootChildren = altitudeApp.service.folder.immediateChildren(RequestContext.repository.value.get.rootFolderId, all)
    rootChildren.head.numOfAssets shouldBe 2
    rootChildren.last.numOfAssets shouldBe 10

    val rootChildren2 = altitudeApp.service.folder.immediateChildren(RequestContext.repository.value.get.rootFolderId)
  ...
  test("Rename asset and attempt to rename a recycled asset") {
    ...
    var updatedAsset: Asset = altitudeApp.service.library.renameAsset(asset.persistedId, "newName")
    updatedAsset.fileName shouldBe "newName"

    // get the asset again to make sure it has been updated
    updatedAsset = altitudeApp.service.library.getById(asset.persistedId)
    updatedAsset.fileName shouldBe "newName"

    // attempt to rename a recycled asset
    asset = altitudeApp.service.library.recycleAsset(asset.persistedId)

    intercept[IllegalOperationException] {
      altitudeApp.service.library.renameAsset(asset.persistedId, "newName2")
    }
  }

  test("Move recycled asset to folder") {
    val asset: Asset = testContext.persistAsset()
    altitudeApp.service.asset.query(new Query()).records.length shouldBe 1
    altitudeApp.service.asset.queryRecycled(new Query()).records.length shouldBe 0
    altitudeApp.service.library.recycleAsset(asset.persistedId)
    altitudeApp.service.asset.queryRecycled(new Query()).records.length shouldBe 1
    ...
    altitudeApp.service.library.query(
      new Query(Map(C.Asset.FOLDER_ID -> folder1.persistedId))
    ).records.length shouldBe 1

    val all = altitudeApp.service.folder.repositoryFolders()

    (altitudeApp.service.folder.getByIdWithChildAssetCounts(folder1.persistedId, all): Folder).numOfAssets shouldBe 1
  }
  ...
  */
    ...
    altitudeApp.service.asset.add(new Asset(
  ...
  test("Move asset to a different folder") {
    ...
    */
    ...
    altitudeApp.service.library.query(new Query()).isEmpty shouldBe true
  ...
  test("Recycle asset") {
    ...
    altitudeApp.service.user.switchContextToUser(user2)

    testContext.persistAsset(user = Some(user2))

    // FIRST USER
    switchContextUser(testContext.users.head)
    altitudeApp.service.asset.query(new Query()).records.length shouldBe 2

    val asset: Asset = altitudeApp.service.asset.query(new Query()).records.head
    altitudeApp.service.library.recycleAsset(asset.persistedId)
  ...
  test("Restore recycled asset") {
    ...
    altitudeApp.service.asset.query(new Query()).isEmpty shouldBe false
  }
  ...
  test("Restore recycled asset to non-existing folder") {
    ...
    intercept[NotFoundException] {
      altitudeApp.service.library.moveAssetToFolder(asset.persistedId, "bad")
    }
  }
  ...
  test("Recycle folder assets") {
    ...
    altitudeApp.service.library.deleteFolderById(folder2.persistedId)

    asset1 = altitudeApp.service.library.getById(asset1.persistedId)
    asset2 = altitudeApp.service.library.getById(asset2.persistedId)
    asset3 = altitudeApp.service.library.getById(asset3.persistedId)
  ...
  test("Restore an asset that was imported again") {
    ...
    altitudeApp.service.library.add(asset)

    // now restore the previously deleted copy into itself
    intercept[DuplicateException] {
      altitudeApp.service.library.restoreRecycledAsset(asset.persistedId)
    }
  }

  test("Deleting a folder recycles all assets and marks folder as recycled") {
    val folder1: Folder = altitudeApp.service.library.addFolder(
      "folder")
    val folder1_1: Folder = altitudeApp.service.library.addFolder(
    ...
    (altitudeApp.service.folder.getById(folder1.persistedId): Folder).isRecycled shouldBe  true

    // Folder 1_1 should stay as recycled, as it has an asset
    (altitudeApp.service.folder.getById(folder1_1.persistedId): Folder).isRecycled shouldBe  true
  ...
  test("Deleting a folder referenced by recycled assets marks folder as recycled") {
    ...
    altitudeApp.service.library.recycleAsset(asset1.persistedId)
    altitudeApp.service.library.recycleAsset(asset2.persistedId)
  ...
  test("Restoring an asset into a recycled folder should recreate the folder") {
    ...
    val deletedFolder: Folder = altitudeApp.service.folder.getById(folder1.persistedId)
    deletedFolder.isRecycled shouldBe true

    altitudeApp.service.library.restoreRecycledAsset(asset.persistedId)

    val restoredFolder: Folder = altitudeApp.service.folder.getById(folder1.persistedId)
