...
@DoNotDiscover class SearchTests(val config: Map[String, String]) extends IntegrationTestCore {
  test("empty search") {
    val assets = altitude.service.library.search(new Query(USER)).records
  ...
  test("search root folder") {
    ...
    val assets = altitude.service.library.search(Query(USER)).records
  ...
  test("search uncategorized folder") {
    ...
    val query = Query(USER, params = Map(C("Api.Folder.QUERY_ARG_NAME") -> Folder.UNCATEGORIZED.id.get))
  ...
  */
    ...
    altitude.service.library.search(
      Query(USER, params = Map(C("Api.Folder.QUERY_ARG_NAME") -> folder1_2.id.get))
    ).records.length should be(1)

    altitude.service.library.search(
      Query(USER, params = Map(C("Api.Folder.QUERY_ARG_NAME") -> folder1_1.id.get))
    ).records.length should be(1)
  ...
  test("folder filtering") {
    ...
    */
    ...
    altitude.service.library.search(
      Query(USER, params = Map(C("Api.Folder.QUERY_ARG_NAME") -> folder2_1.id.get))
    ).records.length shouldEqual 2
  ...
  test("pagination") {
    ...
    val q2 = Query(USER, rpp = 2, page = 2)
    ...
    val q3 = Query(USER, rpp = 2, page = 3)
    ...
    val q4 = Query(USER, rpp = 2, page = 4)
    ...
    val q5 = Query(USER, rpp = 6, page = 1)
    ...
    val q6 = Query(USER, rpp = 20, page = 1)
