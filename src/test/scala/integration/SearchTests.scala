...
@DoNotDiscover class SearchTests(val config: Map[String, String]) extends IntegrationTestCore {
  test("empty search") {
    val assets = altitude.service.library.search(new Query(currentUser.get)).records
  ...
  test("search root folder") {
    ...
    val asset = new Asset(
      userId = currentUser.get.id.get,
      assetType = mediaType,
      path = "path",
      md5 = "md5",
      folderId = currentUser.get.uncatFolderId,
      sizeBytes = 1L)
    altitude.service.asset.add(asset)

    val assets = altitude.service.library.search(Query(currentUser.get)).records
  ...
  test("search uncategorized folder") {
    ...
    val query = Query(currentUser.get, params = Map(C.Api.Folder.QUERY_ARG_NAME -> currentUser.get.uncatFolderId))
  ...
  */
    ...
    altitude.service.asset.add(new Asset(folderId = folder1_1.id.get.toString,
      userId = currentUser.get.id.get, assetType = mediaType, path = Util.randomStr(30), md5 = Util.randomStr(30), sizeBytes = 1L))
    ...
    altitude.service.library.search(
      Query(currentUser.get, params = Map(C.Api.Folder.QUERY_ARG_NAME -> folder1_2.id.get))
    ).records.length should be(1)

    altitude.service.library.search(
      Query(currentUser.get, params = Map(C.Api.Folder.QUERY_ARG_NAME -> folder1_1.id.get))
    ).records.length should be(1)
  ...
  test("folder filtering") {
    ...
    */
    ...
    altitude.service.library.search(
      Query(currentUser.get, params = Map(C.Api.Folder.QUERY_ARG_NAME -> folder2_1.id.get))
    ).records.length shouldEqual 2
  ...
  test("pagination") {
    ...
    val q = Query(currentUser.get, rpp = 2, page = 1)
    ...
    val q2 = Query(currentUser.get, rpp = 2, page = 2)
    ...
    val q3 = Query(currentUser.get, rpp = 2, page = 3)
    ...
    val q4 = Query(currentUser.get, rpp = 2, page = 4)
    ...
    val q5 = Query(currentUser.get, rpp = 6, page = 1)
    ...
    val q6 = Query(currentUser.get, rpp = 20, page = 1)
